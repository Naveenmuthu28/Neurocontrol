...
void setup() {
  ...
  if (networkCount == 0) {
    Serial.println("No Wi-Fi networks found. Restart to try again.");
    return;
  }
  ...
  int networkChoice = 0;
  while (networkChoice == 0) {
    if (Serial.available()) {
      String choiceStr = Serial.readStringUntil('\n');
      choiceStr.trim();
      networkChoice = choiceStr.toInt();
      if (networkChoice < 1 || networkChoice > networkCount) {
  ...
  ssid = WiFi.SSID(networkChoice - 1);
  ...
  // Save credentials if connected successfully
  if (WiFi.status() == WL_CONNECTED) {
